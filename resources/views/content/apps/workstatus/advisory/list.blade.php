    ...
    <section class="app-user-list">
        ...
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Lista Consultivi</h4>
                @if(auth()->user()->can('adivisory_create') || auth()->user()->hasanyrole('super-admin'))
                    <a href="{{route('advisory.import')}}" class="btn btn-primary">{{__('locale.New Advisorys')}}</a>
                @endif
            </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $('.user-list-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('advisorys.head.list') }}",
                columns: [
                    {data: 'ol'},
                    {data: 'created_at'},
                    {data: 'type'},
                    {data: 'total_real'},
                    {data: 'total_theoretical'},
                    {data: 'status'},
